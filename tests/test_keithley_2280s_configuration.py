...
from rig_control.rig_profile import (
    ConnectionDefinition, DeviceBackend, DeviceCapability, DeviceRole, RigProfile,
)
...
def make_profile(connection_type="socket_scpi"):
    parameters = (
        {"host": "192.168.1.51", "port": 5025, "timeout_seconds": 4.0}
        if connection_type == "socket_scpi" else
        {"resource_name": "USB0::0x05E6::0x2280::1234567::INSTR",
         "timeout_seconds": 3.0, "baud_rate": 19200}
    )
    connection = ConnectionDefinition("supply_link", connection_type, parameters)
    role = DeviceRole(
        device_id="supply", friendly_name="2280S supply",
        capability=DeviceCapability.DC_POWER_SUPPLY,
        driver="keithley_2280s", backend=DeviceBackend.REAL,
...
def test_profile_loads_connection_and_rig_limits(connection_type):
    ...
    if connection_type == "socket_scpi":
        assert configuration.connection == SocketScpiConfiguration("192.168.1.51", 5025, 4)
    else:
        assert configuration.connection == VisaScpiConfiguration(
            "USB0::0x05E6::0x2280::1234567::INSTR", 3, 19200,
        )


@pytest.mark.parametrize("changes,message", [
    ({"driver": "keithley_2260b"}, "does not use the keithley_2280s driver"),
...
def test_nonfinite_rig_limits_are_rejected(name, value):
    ...
    with pytest.raises(ValueError, match="finite"):
        Keithley2280SConfiguration(
            "supply", SocketScpiConfiguration("localhost", 5025), PowerSupplyLimits(**values),
        )


@pytest.mark.parametrize("connection_type", ["socket_scpi", "visa_scpi"])
def test_factory_constructs_correct_driver_without_connecting(monkeypatch, connection_type):
    captured = []
    ...
    assert type(supply) is Keithley2280S
    ...
    if connection_type == "socket_scpi":
        assert captured == [((), {"host": "192.168.1.51", "port": 5025, "timeout_seconds": 4})]
    else:
        assert captured == [(("USB0::0x05E6::0x2280::1234567::INSTR",),
