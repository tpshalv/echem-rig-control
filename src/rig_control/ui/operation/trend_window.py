...
class TrendWindow:
    ...
    def __init__(
        ...
        *,
        device_id: str,
        channel: str,
        history_provider: Callable[[], tuple[LiveMeasurementRow, ...]],
        ...
        self._current = ttk.Label(heading, text="No readings")
        self._current.grid(row=1, column=0, sticky="w")
        ...
        self._canvas.grid(row=1, column=0, sticky="nsew", padx=12, pady=(4, 12))
    ...
    def refresh(self) -> None:
        ...
        self._canvas.create_line(left, top, left, bottom, right, bottom, fill=MUTED_TEXT)
        points = [point(index) for index in range(len(readings))]
        if len(points) > 1:
            self._canvas.create_line(*[value for pair in points for value in pair], fill=INFO_TEXT, width=2)
        for reading, (x, y) in zip(readings, points, strict=True):
            colour = INFO_TEXT if reading.quality == "good" else ERROR_TEXT
            self._canvas.create_oval(x - 2, y - 2, x + 2, y + 2, fill=colour, outline=colour)

        self._canvas.create_text(left - 8, top, text=f"{maximum:.6g}", anchor="e")
        self._canvas.create_text(left - 8, bottom, text=f"{minimum:.6g}", anchor="e")
        self._canvas.create_text(
            left,
            bottom + 18,
